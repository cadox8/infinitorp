...
$(document).on("click",".kullanici",function() {
    ...
    if (aranma.durum) {
        ...
        $(".aranma-kutu").html(`
            <div class="aranma-neden"><i class="fas fa-exclamation-triangle fa-lg"></i>${aranma.sebep} meses</div>
    ...
    $.post('http://'+resourceName+'/photo', JSON.stringify({data: secilenCitizenid}), function(cbData) {
        ...
        if (!userBank) { userBank = { bank: 0 }; console.log("No hay datos del banco"); } else { userBank = JSON.parse(userBank).bank; }
        hideUi("#arac-bilgi")
            ...
            for (let index = 0; index < playerVehicleData.length; index++) {
                ...
                $(".arac-bilgi-kutu").append(`
                    <div class="arac-bilgi" data-id="${index}">
                        ...
                        <div class="arac-bilgi-isim">${element.name == 'NULL' ? JSON.parse(element.vehicle).model : element.name}</div>
                    </div>
                `)
            }
            showUi("#arac-bilgi")
...
$(document).on("click","#olay-gir", function() {
    ...
    $(".ceza-hapis").html('<i class="fas fa-exclamation-triangle"></i>0 meses')
...
$(document).on("click","#ceza-bilgi",function() {
    ...
    $(".ceza-hapis").html('<i class="fas fa-exclamation-triangle"></i>' + Math.round(toplamHapis / 100 * indirim) + ' meses')
    ...
    for (let index = 0; index < cbData.length; index++) {
        ...
        htmlOlaylar = htmlOlaylar + `
            <div class="olay" id="olayno-${element.id}">
                ...
                <div class="olay-aciklama">
                    <div class="olay-saat">
                        Multa: ${Math.round(JSON.parse(element.ceza).para / 100 * 100)}$ - Carcel: ${Math.round(JSON.parse(element.ceza).hapis / 100 * 100)} meses
                    </div>
                </div>                
...
$(document).on("click",".aranmabutton", function() {
    ...
    if (neden == "" ) {
        ...
        $(".aranma-kutu").html(`
            <div class="aranma-neden"><i class="fas fa-exclamation-triangle fa-lg"></i>${neden} meses</div>
            <div class="aranma-sag">
