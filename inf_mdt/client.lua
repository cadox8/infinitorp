...
RegisterNUICallback('sorgula', function(data, cb)
    ...
    ESX.TriggerServerCallback("tgiann-mdtv2:sorgula", function(result)
        sorguData = result
        if data.tip == 'arac' then
            for i = 1, #sorguData do
                sorguData[i].name = GetDisplayNameFromVehicleModel(json.decode(result[i].vehicle).model)
            end
        end
    end, data)
...
RegisterNUICallback('cezakaydetclient', function(data)
    ESX.TriggerServerCallback("mdt:identi", function(result)
    end, data.data)
